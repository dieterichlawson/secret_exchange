...
tar -c plaintext to.pem > adressed_plaintext.tar
rm to.pem plaintext
# sign digest of tar
openssl dgst -sha256 -sign $SENDER_PRIVATE_KEY -out signature adressed_plaintext.tar
# add plaintext and signature to new tar file
tar -c adressed_plaintext.tar signature > signed_plaintext.tar
# create random password for aes key
openssl rand -base64 32 > aes_pwd
# encrypt tarfile with aes using aes_pwd
cat aes_pwd | openssl aes-256-cbc -a -base64 -salt -in signed_plaintext.tar -pass stdin -out sign
...
cat plaintext
rm plaintext adressed_plaintext.tar signature signed_plaintext.tar signed_plaintext.enc \
